...
function ChangePlan({ onSelect, onClose }: {
  onSelect: (planName: string, price: string, icon: string, requiredRegions: string[]) => void;
  onClose: () => void;
}) {
  //  useEffect(() => {
  ...
  //   }, []);
  const { selectedPlan, setSelectedPlan, selectedRegions, setSelectedRegions, setBillingCycle, billingCycle } = usePlan()
  ...
  const handleChange = async (planName: string, price: string, icon: string, requiredRegions: string[]) => {
    ...
    if (reqregions > 0 && planRegions.length === 0) {
            ...
            return (
              ...
              >
                <div className={`bg-gradient-to-br ${plan.color} p-6`}
                  ...
                  <div className="mb-3">
                    <span className="text-3xl font-bold text-gray-900">{plan.name === 'Corporate' ? ' ' : `₹${isHalfYearly ? plan.annualdisprice : plan.price}`}</span>
                    <span className="text-sm text-gray-500">{plan.name === 'Corporate' ? ' ' : `/ ${isHalfYearly ? plan.annual : plan.period}`}</span>
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {(plan.name !== 'Corporate' && plan.name !== 'Trial') &&
                        <div className="bg-green-100 text-green-800 text-base p-1 rounded-lg">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <button
                    onClick={() => {
                      handleChange(plan.name, (isHalfYearly ? plan.annualdisprice : plan.price) || "", plan.icon, regions);
                    }}
