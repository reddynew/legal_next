...
export default function EasyForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await contactService.submitCase(newEntry);
      if (data.success) {
        ...
        toast.error("Sorry, case could not be submitted!");
      }
    } catch (error) {
      console.error(error);
      toast.error("Sorry, case could not be submitted!");
  ...
  return (<div className="min-h-screen bg-white relative">
    <section className="px-4 border">
      <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12">
        {/* Header */}
        <div className="sm:sticky top-30 sm:h-[700px] sm:flex justify-center items-center">
          <div className="text-center px-4">
            <h1 className="text-4xl md:text-5xl font-black bg-black bg-clip-text text-transparent mb-4 leading-tight">Get Legal Help</h1> <p className="text-xl md:text-2xl text-gray-600 font-medium max-w-2xl mx-auto leading-relaxed">
              Connect with verified advocates for your legal needs. Fast, secure, and confidential. </p> <h3 className="text-xl font-bold text-black mt-4">Available 24/7</h3>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4">
                <button
                  type="button"
                  onClick={handleReset}
                  className="flex h-[50px] py-2 px-4 rounded-2xl border-2 border-gray-300 text-gray-800 bg-white font-bold text-lg gap-2 justify-center items-center transition-all duration-200 hover:scale-105 active:scale-95 hover:bg-gray-50"
                >
                  Reset Form
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className={`flex h-[50px] py-2 px-4 rounded-2xl font-black border-2 border-transparent text-white text-lg items-center justify-center gap-2 transition-all duration-200 bg-black hover:scale-105 active:scale-95 ${submitting ? 'opacity-70 cursor-not-allowed' : 'hover:bg-black/90'}`}
                >
                  {submitting ? (
                    <>
